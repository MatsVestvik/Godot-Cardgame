...
func _run_test() -> void:
	...
	assert(deck.get_remaining_count() == 10, "Deck has 10 cards")
	assert(energy_mgr.current_energy == 3, "Player starts with 3 energy")
	assert(hand.get_cards().size() == 0, "Hand starts empty")

	# 2. Draw card into hand
	...
	assert(deck.get_remaining_count() == 9, "Deck has 9 remaining")

	# 3. Play card to table (costs 1 energy, triggers ability to draw 1 card!)
	card1._pickup_card()
	...
	# Ability triggered! Draw 1 card was executed, so hand drew 1 card!
	assert(hand.get_cards().size() == 1, "Hand should have 1 card drawn by ability")
	assert(deck.get_remaining_count() == 8, "Deck should have 8 remaining after ability draw")
	print("Draw card played successfully: cost 1 energy and drew 1 card!")

	# 4. Play another card (energy drops to 1, draws 1 card)
	...
	assert(hand.get_cards().size() == 1, "Hand drew 1 card from ability")
	assert(deck.get_remaining_count() == 7, "Deck has 7 remaining")

	# 5. Play another card (energy drops to 0, draws 1 card)
	...
	# 7. Test resetting energy (End Turn)
